...
class Trader:
    ...
    def __init__(self):
        ...
        self.last_mid_price = {'RAINFOREST_RESIN': 10000, 'KELP': 4935, "SQUID_INK": 1839}
        self.last_ema_price = {'RAINFOREST_RESIN': 10000, 'KELP': 4900, "SQUID_INK": 1839}
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'KELP':
                ...
                # Calculate the mid price of the RAINFOREST_RESIN market
            ...
            if product == 'SQUID_INK':
                ...
                if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                    ...
                    avg_buy_price = sum(i * vol for i, vol in order_depth.buy_orders.items()) / sum(order_depth.buy_orders.values())
                    avg_sell_price = sum(i * -vol for i, vol in order_depth.sell_orders.items()) / sum(-v for v in order_depth.sell_orders.values())
                ...
                if len(order_depth.sell_orders) > 0 and len(order_depth.buy_orders) > 0:
                    ...
                    if best_ask <= acceptable_price:
                        orders.append(Order(product, best_ask, int(np.minimum(-best_ask_volume, legal_buy_vol))))
                        legal_buy_vol -= int(np.minimum(-best_ask_volume, legal_buy_vol))

                    if best_bid >= acceptable_price:
                        orders.append(Order(product, best_bid, int(np.maximum(-best_bid_volume, legal_sell_vol))))
                        legal_sell_vol -= int(np.maximum(-best_bid_volume, legal_sell_vol))
