        ...
        for product in state.order_depths.keys():
            if product == 'PEARLS':
                ...
                if state.timestamp > start_trading:
                    model = ARIMA(4,0,1)
                    pred = model.fit_predict(price_history_pearl)
                    ...
                    if len(order_depth.sell_orders) > 0:
                        ...
                        if best_ask+spread < forecasted_price and np.abs(best_ask_volume) > 0:
                    ...
                    if len(order_depth.buy_orders) != 0:
                        ...
                        if best_bid-spread > forecasted_price and best_bid_volume > 0:
