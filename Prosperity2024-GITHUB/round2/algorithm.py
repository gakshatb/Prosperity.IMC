    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == 'KELP':
                ...
                self.fair_prices[product] = fair_price
                ...
                ob_info = state.observations.conversionObservations.get(product)
                ...
                if ob_info is not None:
                    ...
                    logger.print(
                        f"Observation bidPirce: {observe_bid}, askPrice: {observe_ask}, transportFees: {observe_transport}, "
                        f"exportTariff: {observe_export}, importTariff: {observe_import}, sunlight: {observe_sunlight}, "
                        f"humidity: {observe_humidity}"
                    )
                    ...
                    predict_price = (
                        np.dot(coefficient, np.transpose(variable))
                        + intercept
                        + (observe_bid + observe_ask) / 2
                    )
                    ...
                    bid_pr_tmp, _ = max(sell_list, key=lambda x: x[1])
                    bid_pr = min(bid_pr_tmp + 1, price)
                    orders[product].append(Order(product, bid_pr, self.pos_limit[product] - cpos))
                    ...
                    orders[product].append(Order(product, sell_pr, -self.pos_limit[product] - cpos))
                    self.fair_prices[product] = fair_price

                else:
                    logger.print(f"[WARNING] No conversionObservation found for product: {product}. Skipping advanced trading logic.")
                ...
                best_ask = min([ask for ask, _ in sell_orders], default=float('inf'))
                best_bid = max([bid for bid, _ in buy_orders], default=0)

                fair_price = (best_ask + best_bid) / 2 if best_ask < float('inf') and best_bid > 0 else 10000

                position = self.position[product]
                pos_limit = self.pos_limit[product]

                for price, volume in sell_orders:
                    if price < fair_price:
                        volume = min(-volume, pos_limit - position)
                        orders[product].append(Order(product, price, volume))
                        position += volume

                for price, volume in buy_orders:
                    if price > fair_price:
                        volume = max(-volume, -pos_limit - position)
                        orders[product].append(Order(product, price, volume))
                        position += volume
                self.fair_prices[product] = fair_price
                ...
                price_c = self.fair_prices.get("CROISSANTS", 0)
                price_j = self.fair_prices.get("JAMS", 0)
                price_d = self.fair_prices.get("DJEMBES", 0)
                basket_value = 6 * price_c + 3 * price_j + 1 * price_d

                position = self.position[product]
                pos_limit = self.pos_limit[product]

                for price, volume in sell_orders:
                    if price < basket_value:
                        volume = min(-volume, pos_limit - position)
                        orders[product].append(Order(product, price, volume))
                        position += volume
                ...
                # Basket of 4 CROISSANTS, 2 JAMS
                orders[product] = []
                sell_orders = list(order_depth.sell_orders.items())
                buy_orders = list(order_depth.buy_orders.items())
                price_c = self.fair_prices.get("CROISSANTS", 0)
                price_j = self.fair_prices.get("JAMS", 0)
                basket_value = 4 * price_c + 2 * price_j
