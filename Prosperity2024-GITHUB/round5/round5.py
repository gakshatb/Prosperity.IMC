...
class Trader:
    ...
    def cal_actual_price(self, S, time):
        ...
        # T = 250 - int(os.environ['PROSPERITY2BT_DAY']) - time / 1000000
        T = 246  - time / 1000000
        r = 0
        K = 10000
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for key in state.own_trades.keys():
            if key == 'COCONUT':
                for i in range(len(state.own_trades[key])):
                    if state.own_trades[key][i].buyer == 'Raj':
                        self.future_adjust = [state.own_trades[key][0].timestamp + 2000, midprice_coconut + 3, 1]
                    elif state.own_trades[key][i].seller == 'Raj':
                        self.future_adjust = [state.own_trades[key][0].timestamp + 2000, midprice_coconut - 3, -1]
        for key in state.market_trades.keys():
            if key == 'COCONUT':
                for i in range(len(state.market_trades[key])):
                    if state.market_trades[key][i].buyer == 'Raj':
                        self.future_adjust = [state.market_trades[key][0].timestamp + 2000, midprice_coconut + 3, 1]
                    elif state.market_trades[key][i].seller == 'Raj':
                        self.future_adjust = [state.market_trades[key][0].timestamp + 2000, midprice_coconut - 3, -1]
        
        if len(self.future_adjust) > 0:
            timestamp, val, stat = self.future_adjust
            if timestamp - time > 1500:
                if stat == 1 and can_buy_coconut:
                    if val > midprice_coconut + 2:
                        orders['COCONUT'].append(Order('COCONUT', floor(midprice_coconut + 1), self.pos_limit['COCONUT'] - self.position['COCONUT']))
                elif stat == -1 and can_sell_coconut:
                    if val < midprice_coconut - 2:
                        orders['COCONUT'].append(Order('COCONUT', floor(midprice_coconut), -self.pos_limit['COCONUT'] - self.position['COCONUT']))
            elif timestamp > time:
                if stat == 1 and val < midprice_coconut:
                    orders['COCONUT'].append(Order('COCONUT', floor(midprice_coconut), self.pos_limit['COCONUT'] - self.position['COCONUT']))
                elif stat == -1 and val > midprice_coconut:
                    orders['COCONUT'].append(Order('COCONUT', floor(midprice_coconut) + 1, -self.pos_limit['COCONUT'] - self.position['COCONUT']))
        ...
        if fair_price_coconut_coupon > midprice_coconut_coupon + 5:
            num_buy_coconut_coupon = floor(min(self.pos_limit['COCONUT_COUPON'] - self.position['COCONUT_COUPON'], (fair_price_coconut_coupon - midprice_coconut_coupon) * 8))
        ...
        if fair_price_coconut_coupon < midprice_coconut_coupon - 5:
            num_sell_coconut_coupon = floor(max(-self.pos_limit['COCONUT_COUPON'] - self.position['COCONUT_COUPON'], (fair_price_coconut_coupon - midprice_coconut_coupon) * 8))
