...
def compute_fair_value(order_depth):
    """
    Computes the fair value as the mid-price between best bid and best ask.
    If only one side is available, uses that price.
    """
...
def position_limit_reached(product, current_position, position_limits):
    """
    Checks if the position limit has been reached for a given product.
    Returns True if buying or selling would exceed the limit.
    """
    limit = position_limits.get(product, 0)
    return abs(current_position) >= limit
...
class MacaronStrategy:
    ...
    POSITION_LIMIT = 75
    CONVERSION_LIMIT = 10
    STORAGE_COST = 0.1  # per timestamp per unit (net long only)
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        conversion_observation: ConversionObservation = state.observations.conversion_observations.get(self.PRODUCT)

        # Fetch conversion prices and fees
        ask_price = conversion_observation.ask_price
        bid_price = conversion_observation.bid_price
        transport_fees = conversion_observation.transport_fees
        import_tariff = conversion_observation.import_tariff
        export_tariff = conversion_observation.export_tariff

        total_buy_cost = ask_price + transport_fees + import_tariff
        total_sell_revenue = bid_price - transport_fees - export_tariff

        # Simple strategy:
        # If market sell price > conversion buy price + fees => buy from conversion, sell to market
        # If market buy price < conversion sell price - fees => buy from market, sell to conversion

        best_ask = min(order_depth.sell_orders.keys()) if order_depth.sell_orders else None
        best_bid = max(order_depth.buy_orders.keys()) if order_depth.buy_orders else None

        if best_ask is not None and best_ask < total_sell_revenue:
            # Buy from market and sell via conversion
            volume = min(-order_depth.sell_orders[best_ask], self.CONVERSION_LIMIT, self.POSITION_LIMIT - position)
            if volume > 0:
                orders.append(Order(self.PRODUCT, best_ask, volume))

        if best_bid is not None and best_bid > total_buy_cost:
            # Sell to market and buy via conversion
            volume = min(order_depth.buy_orders[best_bid], self.CONVERSION_LIMIT, position + self.POSITION_LIMIT)
            if volume > 0:
    ...
    def initialize_strategies(self, products_with_limits):
        for product, limit in products_with_limits.items():
            if product == "RAINFOREST_RESIN":
                ...
                self.strategy_map[product] = MacaronStrategy()
            else:
