...
class Trader:
    ...
    coupon_price = 637.63
    coupon_implied_vol = 0.19325

    coconut_delta = 0.532
    coconut_theta = -1.298
    days_till_expiry = 250
    day_coco_curr = 1
    last_timestamp = 0
    buyvolgth = 1000000
    prevbuyVolgth = 1
    ...
    def black_scholes_price(self, marketPrice):
        return self.BS_CALL(marketPrice, self.coconut_strike, ((self.days_till_expiry - (self.last_timestamp/1000000)))/365, 0.00, self.coupon_implied_vol)
    ...
    def compute_orders_coupons(self, order_depth):
        ...
        acceptable_coupon_price = self.black_scholes_price(mid_price['COCONUT'])  + self.coconut_delta*(mid_price['COCONUT']-self.coconut_strike)
        theta_comp = (self.coconut_theta)*(self.last_timestamp/1000000)
        acceptable_coupon_price += theta_comp
        if mid_price['COCONUT_COUPON'] > acceptable_coupon_price:
            acceptable_coupon_price = (mid_price['COCONUT_COUPON']+1)
        elif acceptable_coupon_price > mid_price['COCONUT_COUPON']:
            acceptable_coupon_price = mid_price['COCONUT_COUPON']
        print("acceptable_coupon_price", acceptable_coupon_price)


        if self.buyprevVol:
            self.buyvolgth = (vol_buy['COCONUT_COUPON'])/self.buyprevVol
            ddxb = 1
            if self.prevbuyVolgth:
                ddxb = (self.buyvolgth - self.prevbuyVolgth)/self.prevbuyVolgth
            self.prevbuyVolgth = self.buyvolgth
            self.buyprevVol = vol_buy['COCONUT_COUPON']

        if self.sellprevVol:
            self.sellvolgth = (vol_sell['COCONUT_COUPON'])/self.sellprevVol
            ddxs = 1
            if self.prevsellVolgth:
                ddxs = (self.sellvolgth - self.prevsellVolgth)/self.prevsellVolgth
            self.prevsellVolgth = self.sellvolgth
            self.sellprevVol = vol_sell['COCONUT_COUPON']

        vol, vol1 = (0, 0)
        ddxb, ddxs = (0, 0)

        if(best_sell['COCONUT_COUPON'] < acceptable_coupon_price and ddxb > 0.02): #vol_buy['COCONUT_COUPON']*0.7 > vol_sell['COCONUT_COUPON']
        ...
        if(worst_buy['COCONUT_COUPON'] > acceptable_coupon_price and ddxs > 0.02): #vol_sell['COCONUT_COUPON']*0.8 > vol_buy['COCONUT_COUPON']
            ...
            if ddxs > 0.1:
                vol1 = -self.POSITION_LIMIT['COCONUT_COUPON'] - self.position['COCONUT_COUPON']
